...
export default function RelationForm({ initial, onSubmit, onCancel }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (isEditing) {
      // При редактировании отправляем только даты
  ...
  return (
    <div className="bg-gray-50 p-5 rounded-lg border border-gray-200 mb-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {!isEditing && (
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">ID человека</label>
            <input type="number" value={relativeId} onChange={(e) => setRelativeId(e.target.value)} className="w-full bg-white border border-gray-200 rounded-lg px-4 py-3 text-gray-700 placeholder-gray-400 focus:outline-none focus:border-caramel focus:ring-1 focus:ring-caramel transition-colors" required />
          </div>
        )}
        {!isEditing && (
          <div>
        ...
        {(showDates || isEditing) && (
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Дата развода <span className="text-gray-400 font-normal">(необязательно)</span></label>
